...
function createMatchWhatsappMessage(products: Array<{ titleAr: string }>) {
  const names = products.map((product) => `- ${product.titleAr}`).join('\n');
  return `أهلًا، حابة أسأل عن الترشيحات دي:\n${names}\nهل متاحين؟ والتفاصيل إيه؟`;
}
