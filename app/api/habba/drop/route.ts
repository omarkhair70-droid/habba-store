...
function createDropWhatsappMessage(dropNameAr: string, products: Array<{ titleAr: string }>) {
  const names = products.map((product) => `- ${product.titleAr}`).join('\n');
  return `أهلًا، حابة أسأل عن الـ Drop ده:\n${dropNameAr}\n${names}\nهل متاحين؟ والتفاصيل إيه؟`;
}
