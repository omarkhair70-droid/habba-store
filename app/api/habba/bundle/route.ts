...
function createBundleWhatsappMessage(products: Array<{ titleAr: string }>) {
  const names = products.map((product) => `- ${product.titleAr}`).join('\n');
  return `أهلًا، حابة أسأل عن الباندل ده:\n${names}\nهل متاحين؟ والتفاصيل إيه؟`;
}

export async function POST(req: NextRequest) {
  const ip = req.headers.get('x-forwarded-for')?.split(',')[0]?.trim() || 'unknown'; if (!checkLimit(ip)) return NextResponse.json({ error: 'طلبات كتير في وقت قصير، جربي بعد شوية.' }, { status: 429 });
  ...
  return NextResponse.json({ bundleNameAr: ai.ok && ai.data.bundleNameAr ? ai.data.bundleNameAr : 'باندل حبّة جاهز', summaryAr: ai.ok && ai.data.summaryAr ? ai.data.summaryAr : 'اختيارات مناسبة لبعض حسب مودك.', products, whatsappMessageAr: ai.ok && ai.data.whatsappMessageAr ? ai.data.whatsappMessageAr : createBundleWhatsappMessage(products), source: ai.ok ? 'ai' : ai.error === 'local_mode' ? 'local' : 'fallback', debug: { interpretedMoodAr: body.bundleIntent, suggestedFilterKey: body.includeType } });
}
